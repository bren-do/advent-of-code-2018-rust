...
fn reaction_for_letter (input: &str, reacting_letter: u8) -> usize {
    let mut output_bytes = &mut input.trim().as_bytes().iter()
        .map(|&x| x) //y tho?
...
fn min_reaction_length(input: &str) -> usize {
    ...
    reacting_letters.as_bytes().iter()
...
fn main() {
    ...
    let real_input = fs::read_to_string("input.txt")
        .expect("Something went wrong reading the file");

    let letters = &mut real_input.trim().as_bytes().to_vec();
    let output_size = reaction_size(letters);
